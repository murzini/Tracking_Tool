...
- **Lifecycle:** zero-interaction visits are recorded (bounce); the session id persists in localStorage and **resumes within X** (30 s normal / 2 s autotest), else a new session starts. Session signals written: `outcome` (`advanced` / `completed` / `abandoned` / `in-progress`), `exit_reason` (`idle` / `nav-click` / `back` / `left-browser`, non-completed only), `step_active_ms` / `step_idle_ms` (`active + idle = duration`), `sampling_rate`. A sampling gate (cookie `m1.heatmap.sampled`) can suppress capture entirely.
