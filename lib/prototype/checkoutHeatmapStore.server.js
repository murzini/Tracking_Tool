...
export async function readCheckoutHeatmapSessions({ schema = SCHEMA } = {}) {
  try {
    const sql = getSql();
...
export async function ingestCheckoutHeatmapBatch({ session, events } = {}) {
  ...
  // than a bare null. The finalize flush (completed/abandoned) carries its own
  // outcome; only the incremental/beacon commits (no outcome, not yet finalized)
  // are stamped here.
  ...
  await sql.query(
    ...
    ON CONFLICT (id) DO UPDATE SET
      ...
      -- M4 Part 6: a terminal outcome (completed/abandoned) wins so a late or
      -- out-of-order in-progress flush never reverts a resolved session.
      -- Otherwise take the incoming outcome, falling back to the current one.
      outcome             = CASE
                              WHEN "${SCHEMA}".sessions.outcome IN ('completed','abandoned')
...
export async function countCheckoutHeatmapSessions({ schema = SCHEMA } = {}) {
  try {
    const sql = getSql();
    const rows = await sql.query(`SELECT COUNT(*) AS count FROM "${schema}".sessions`);
...
export async function clearCheckoutHeatmapSessions({ schema = SCHEMA } = {}) {
  const sql = getSql();
...
// M6.1 — bulk insert for the simulation generator. Writes many pre-finalized
// sessions + their events in a bounded number of round-trips (one multi-row
// INSERT per session chunk + one multi-row INSERT per event chunk) rather than
// looping ingestCheckoutHeatmapBatch per session. Idempotent: conflicts on id
// are silently skipped. Sessions must already be finalized (outcome set,
// finalizedAt set) before passing here.
export async function bulkInsertCheckoutHeatmapSessions(sessions, { schema = SCHEMA } = {}) {
  if (!sessions.length) return 0;
  const sql = getSql();
  const SESSION_CHUNK = 50;  // 50 × 21 params = 1050, well under the 65535 limit
  const EVENT_CHUNK = 12000; // 12000 × 5 params = 60000, under the 65535 limit

  for (let i = 0; i < sessions.length; i += SESSION_CHUNK) {
    const normalized = sessions.slice(i, i + SESSION_CHUNK).map(s =>
      normalizeCheckoutHeatmapSession(s)
    );
    ...
    for (const s of normalized) {
      sRows.push(`($${p++},$${p++},$${p++},$${p++},$${p++},$${p++},$${p++},$${p++},$${p++},$${p++},$${p++},$${p++},$${p++},$${p++},$${p++},$${p++},$${p++},$${p++},$${p++},$${p++},$${p++})`);
      sParams.push(
        s.id, s.version, s.step, s.sku, s.view,
        s.viewport.width, s.viewport.height,
        s.startedAt, s.lastInteractionAt, s.finalizedAt,
        s.coordinateScope, s.inactivityMs, s.durationMs ?? null,
        s.clickCount ?? 0, s.interactionCount ?? 0,
        s.outcome ?? null, s.exitReason ?? null,
        s.stepActiveMs ?? null, s.stepIdleMs ?? null,
        s.samplingRate ?? 1, s.visitorId ?? null
      );
    }
    await sql.query(
      `INSERT INTO "${schema}".sessions (
        ...
        click_count, interaction_count, outcome, exit_reason,
        step_active_ms, step_idle_ms, sampling_rate, visitor_id
      ) VALUES ${sRows.join(',')} ON CONFLICT (id) DO NOTHING`,
      sParams
    );

    // Collect events from this session chunk
    const allEvents = [];
    for (const s of normalized) {
      for (const ev of s.events) {
        const { id, type, timestamp, ...detail } = ev;
        allEvents.push([id, s.id, type, timestamp, JSON.stringify(detail)]);
      }
    }

    // Bulk event insert, chunked under the param limit
    for (let j = 0; j < allEvents.length; j += EVENT_CHUNK) {
      const batch = allEvents.slice(j, j + EVENT_CHUNK);
      const eRows = [];
      const eParams = [];
      let ep = 1;
      for (const [id, sessionId, type, timestamp, detail] of batch) {
        eRows.push(`($${ep++},$${ep++},$${ep++},$${ep++},$${ep++}::jsonb)`);
        eParams.push(id, sessionId, type, timestamp, detail);
      }
      await sql.query(
        `INSERT INTO "${schema}".events (id, session_id, type, timestamp, detail)
         VALUES ${eRows.join(',')} ON CONFLICT (id) DO NOTHING`,
        eParams
      );
    }
  }
...
// Sessions that completed already carry an outcome and finalized_at, so
