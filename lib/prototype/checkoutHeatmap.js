...
// M4 Part 5 — step outcome + exit reason. M6 unifies outcomes: `completed` means
// "completed this step" (applied uniformly to all steps); `abandoned` is a drop-off.
// Exit reason only applies to a drop-off — how the visitor left a step they did not complete.
...
// M4 Part 3: append any event type (mouse-move, scroll, …). `resetActivity`
// controls whether this counts as user activity: true (default) updates
// `lastInteractionAt` so the inactivity timer treats it as activity; false stores
// the event without touching `lastInteractionAt` (used for passive observer events
// such as element-visible / element-hidden that must not keep the session alive).
export function appendCheckoutHeatmapEvent(session, event, { resetActivity = true } = {}) {
  if (!session || !event) return session;
  const normalizedEvent = normalizeCheckoutHeatmapEvent(event);
  const base = resetActivity
...
export function normalizeCheckoutHeatmapSession(session) {
  ...
  return {
    ...
    visitorId: source.visitorId ? String(source.visitorId) : null,
