...
// Click-weight budget per anchor-id prefix. Each anchor receives its type's
// budget divided by the number of anchors of that type, so collective weight
// per type matches the stated percentages.
const CLICK_TYPE_WEIGHTS = {
  cta:               0.30,
  toggle:            0.15,
  dropdown:          0.12,
  text:              0.08,
  ...
  accordion:         0.015,
  area:              0.005,
  "tooltip-content": 0.005,
};

const _piRaw = CHECKOUT_ELEMENT_REGISTRY
  .filter((e) => e.removedAt === null && e.steps.includes("personal-info"));

const _typeCounts = _piRaw.reduce((acc, e) => {
  const p = e.id.split(":")[0];
  acc[p] = (acc[p] || 0) + 1;
  return acc;
}, {});

// Weighted anchor list — used by genClicks via pickWeighted.
const PI_ANCHOR_WEIGHTS = _piRaw.map((e) => {
  const p = e.id.split(":")[0];
  const budget = CLICK_TYPE_WEIGHTS[p] ?? 0.01;
  return { id: e.id, weight: budget / _typeCounts[p] };
});
...
function genMoves(count, maxX, maxY, baseTs) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    y = Math.max(10, Math.min(maxY - 10, y + ri(-80, 80)));
    events.push({
      id: genId("sim_ev"),
      type: "mouse-move",
      x: Math.round(x),
...
export function generateSimulatedSessions({ count = TOTAL_SESSIONS } = {}) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const maxX = vpWidth - 10;
    const maxY = isDesktop ? 2500 : 3500;

    const events = [
      ...genClicks(clickCount, PI_ANCHOR_WEIGHTS, clickTs),
