...
import { resolveSamplingRate } from "./checkoutHeatmapSampling";
...
import { isCaptureWindowOpen } from "./captureWindowCheck";
import { resolveExitReason as _resolveExitReason } from "./exitReasonResolver";
...
export function useCheckoutHeatmapCapture({ enabled, sku, step, automation = false, inactivityMs } = {}) {
  useEffect(() => {
    ...
    // Sampling gate (per-SESSION). The probabilistic decision needs the runtime config
    // rate, which is only known after the background fetch below. Synchronously we honor
    // only a deterministic query-param/env override: an explicit 0% stops capture from
    // starting at all (tests/manual). Otherwise capture starts and the background handler
    // applies the per-session probability once the config rate has loaded.
    const syncRate = resolveSamplingRate(window.location.search, undefined);
    if (syncRate <= 0) return undefined;

    // M6: drop-off threshold. Defaults to the synchronous value (automation / prop
    // override / manual default); the background config fetch below may replace it
    // with the admin-configured value once it resolves.
    let threshold = getCheckoutHeatmapInactivityMs({ automation, overrideMs: inactivityMs });
    ...
    const resolveExitReason = ({ unload = false } = {}) =>
      _resolveExitReason({ sawBack, lastNavClickAt: lastNavClick?.at ?? null, now: Date.now(), unload });
    ...
    const saveSession = async ({ force = false, outcome } = {}) => {
      ...
      try {
        ...
        persistActiveSession(session);
        // On a failed save, re-arm the finalize timer WITHOUT calling touchResume —
        // a failed HTTP write is not real user activity and must not refresh the
        // resume window (which would cause Test 44 / visibility-event race).
        if (!outcome) {
          clearFinalizeTimer();
    ...
    //   • the per-session sampling decision says "not sampled" (0%, or the coin flip
    //     for an intermediate rate; 100% always passes; a resumed session is kept)
    ...
    fetchRuntimeConfig().then((config) => {
      ...
      // Per-SESSION sampling: decided here because the effective rate (query-param →
      // runtime config → env → default) is only fully known once the config has loaded.
      // Each session (one step visit) is sampled independently — we measure per-step
      // conversion, not whole journeys — so a NEW session flips its own coin at that rate;
      // a RESUMED session keeps its prior in-sample decision (continuity within window X).
      // 0%/100% are deterministic so the gate stays testable. The effective rate is stored
      // on the session for report-time count scaling.
      const effectiveRate = resolveSamplingRate(window.location.search, runtimeConfig?.samplingRate);
      session.samplingRate = effectiveRate;
      if (effectiveRate <= 0) { abort(); return; }
      if (!resumedId && effectiveRate < 1 && !(Math.random() < effectiveRate)) { abort(); return; }

      // M6: apply the admin-configured drop-off threshold. An explicit prop or
      // automation override still wins (tests rely on this). Updating the session
      // field keeps the persisted threshold accurate; an already-pending finalize
      // timer is rescheduled so the new value takes effect this visit.
      if (!automation && inactivityMs == null) {
        const cfgMs = Number(runtimeConfig?.inactivityMs);
        if (Number.isFinite(cfgMs) && cfgMs > 0 && cfgMs !== threshold) {
          threshold = cfgMs;
          session.inactivityMs = cfgMs;
...
function findNearestAnchor(event, surfacePoint) {
  ...
  // Always store the actual offset from the anchor's centre so the dot lands
  // where the user clicked, not at the element centre. This matters for wide
  // elements such as validation-error labels whose centre is far from the
  // left-aligned text the user actually clicked on. For clicks at or near the
  // centre (buttons, inputs) dx/dy ≈ 0 and behaviour is unchanged.
  return {
    id: nearest.id,
    type: nearest.type,
