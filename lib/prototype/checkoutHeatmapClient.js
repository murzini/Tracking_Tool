...
export function useCheckoutHeatmapCapture({ enabled, sku, step, automation = false, inactivityMs } = {}) {
  useEffect(() => {
    ...
    // M4 Part 2: visitor-level sampling gate. If this visit is not sampled,
    // capture is fully off — no session, no listeners, no events.
    const { sampled, rate } = resolveSamplingDecision(window.location.search);
    ...
    const saveSession = async ({ force = false, outcome } = {}) => {
      ...
      const hasClicks = Array.isArray(session.events) && session.events.some((e) => e.type === "click" || e.type === "tap");
      // A drop-off needs at least one click. An explicit outcome (advanced /
      // completed) is a deliberate success finalize triggered by step navigation,
      // so it bypasses the click gate.
      if (!outcome && !hasClicks) return false;
