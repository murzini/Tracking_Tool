...
- M6.1: sim schemas (`heatmap_sim` / `heatmap_test_sim`) added to `scripts/db-setup.mjs`; `resolveHeatmapSchema(source)` enum allowlist in `lib/prototype/db.js`; `readCheckoutHeatmapSessions` / `countCheckoutHeatmapSessions` / `clearCheckoutHeatmapSessions` / `bulkInsertCheckoutHeatmapSessions` parameterized with optional `schema` arg; `checkoutHeatmapSimulator.js` generates ~1500 pre-finalized synthetic sessions; `/api/checkout-heatmap/simulate` route (GET count / auth-gated POST generate / auth-gated DELETE discard); viewer reads `?source=sim` to fetch from the sim schema; dashboard Simulation section (Generate / View Simulation / Discard with confirm overlay); `global-teardown.ts` also wipes `heatmap_test_sim`.
...
- `app/api/checkout-heatmap/simulate/route.js` (M6.1: `GET` count / auth-gated `POST` generate / auth-gated `DELETE` discard — operates on the sim schema only)
- `lib/prototype/checkoutHeatmapSimulator.js` (M6.1: synthetic session generator — ~1500 pre-finalized sessions for `personal-info`, all three event types, fixed outcome/device mix)
...
Two layers from M6.2 onward: **unit tests** (Vitest, fast, no DB or server) run first; **e2e tests** (Playwright, against `localhost:3000`) run after unit tests are green. See `AGENTS.md` → "Unit + e2e test workflow" for the standing convention.

Primary files:
- `vitest.config.ts` (M6.2: Vitest config — Node environment, `tests/unit/**/*.test.ts`)
- `tests/unit/checkoutHeatmap.test.ts` — 34 unit tests (M6.2: pure-logic rules for `checkoutHeatmap.js`)
- `tests/unit/checkoutHeatmapSampling.test.ts` — 7 unit tests (M6.2: `resolveSamplingRate` precedence + clamping)
- `tests/unit/heatmapConfigStore.test.ts` — 5 unit tests (M6.2: `getDefaultHeatmapConfig` defaults + isolation)
...
## M6.1 architecture — heatmap simulation mode

**Status: CLOSED (2026-05-28). All 3 parts delivered. 73/73 active tests passing (Tests 57–63 new, Test 54 updated). All close gates met.** Scope decisions recorded 2026-05-28 (`PRODUCT_OVERVIEW.md` → Future Milestones → M6.1). This section is the architecture + phased implementation plan. Anticipated tech debt is recorded in `PRODUCT_OVERVIEW.md` → Tech Debt → Anticipated (M6.1).

### Goal and boundaries

M6.1 adds a dashboard **Simulation** section that generates ~1500 synthetic sessions on the Personal Information step and previews them in the existing heatmap viewer, so an admin can see the visualisations populated at volume without real traffic. The synthetic data lives in a **separate Postgres schema** so it never mixes with real data and can be wiped without touching it.

- **In scope (generation):** a server-side generator that produces ~1500 finalized synthetic sessions on `personal-info` only, across both views and all three event types (clicks, mouse-move, scroll), following a fixed realistic outcome/exit/device mix (`PRODUCT_OVERVIEW.md` → M6.1 decisions).
- **In scope (isolation):** a dedicated sim schema (`heatmap_sim` live / `heatmap_test_sim` under the test runner) holding the same `sessions`/`events` tables; real `public`/`heatmap_test` data is never read or written by simulation, and discard wipes only the sim schema.
- **In scope (store):** parameterize the store read/clear/insert functions with an optional `schema` arg (default = the env `SCHEMA`), resolved through a small allowlist so no arbitrary schema name can reach SQL.
- **In scope (viewer):** the existing heatmap viewer reads sim data when opened with a `source=sim` enum param; rendering is unchanged (data-source swap only).
- **In scope (dashboard):** a Simulation section between Heatmap and Report — status line, **Generate**, **Discard** (confirm pop-up), and **View Simulation** (opens the viewer fixed to `personal-info` + `source=sim`). Generate/Discard are auth-gated like the config Save.
- **Out of scope (POC):** admin-configurable session count or distributions (hardcoded); simulating steps other than `personal-info`; respecting the capture on/off config toggles (the generator ignores them by design); auth on the read-only sim view (synthetic data leaks nothing — consistent with the public real-data view).

### Schema isolation decision

The sim data is held in its **own schema**, not a `simulated` flag on the existing tables. Rationale: a separate schema makes "discard" a single `DELETE`/`TRUNCATE` against that schema with **zero** risk of touching real rows, and it reuses the exact isolation mechanism already proven for `heatmap_test`. A flag would force every real-data query to remember to filter it out, and one missed filter would silently mix synthetic data into real heatmaps/exports. The sim schema carries the same `sessions` + `events` DDL as the others (added to `scripts/db-setup.mjs`).

**Sim-schema naming + resolution.** The active base schema is env-driven (`HEATMAP_DB_SCHEMA`: `public` live, `heatmap_test` under the runner). The sim schema is its sibling: **`heatmap_sim`** when the base is `public`, **`heatmap_test_sim`** when the base is `heatmap_test` — so the post-suite teardown can wipe the test sim schema and sim runs never pollute live. A `resolveHeatmapSchema(source)` helper maps the **enum** `source ∈ { real (default), sim }` to the concrete schema name and is the *only* place a schema string is produced; it never accepts a raw schema name from the client. This closes the schema-injection surface noted in the anticipated tech debt — the URL/API carries the enum `sim`, not a table-qualified schema name.

### Architecture changes

- **DB setup (`scripts/db-setup.mjs`).** Extend `SCHEMAS` to also create the sim schemas (`heatmap_sim`, `heatmap_test_sim`) with the same `sessions`/`events` tables + indexes. The `heatmap_config` table is not needed in the sim schema (simulation ignores runtime config).
- **Schema resolver + allowlist (`lib/prototype/db.js` or a small new helper).** Add `resolveHeatmapSchema(source)` returning the base schema for `real`/unset and the resolved sim schema for `sim`; reject anything else. Keep the existing `^[a-z_][a-z0-9_]*$` guard as defence-in-depth. `SCHEMA` stays the default for all real-data paths.
- **Parameterized store (`lib/prototype/checkoutHeatmapStore.server.js`).** Give `readCheckoutHeatmapSessions`, `clearCheckoutHeatmapSessions`, and the insert path an optional `schema` argument (default `SCHEMA`); `sessionSelectFrom(schema)` already takes a schema, so the reads are a thin thread-through. Add a **bulk insert** function (e.g. `bulkInsertCheckoutHeatmapSessions(sessions, { schema })`) that writes many sessions + their events in a bounded number of multi-row INSERTs (chunked under the Postgres parameter cap) rather than looping `ingestCheckoutHeatmapBatch` per session — this is what keeps a 1500-session generation from becoming thousands of round-trips (see the synchronous-generator tech-debt item).
- **Generator (`lib/prototype/checkoutHeatmapSimulator.js`, new).** A pure(-ish) module that builds ~1500 synthetic session objects for `personal-info`: assigns `view` (≈60% `desktop_view` / 40% `mobile_view`) with a plausible viewport, an `outcome` (≈65% `abandoned` / 35% `completed`) and, for drop-offs, an `exit_reason` (≈50% idle / 30% left-browser / 15% nav-click / 5% back), `started_at`/`finalized_at` spread over a recent window, and an `events[]` payload: **click** events carrying the rich anchor shape (`{ anchor: { id, dx, dy } }`) sampled from the Personal Information anchors in `CHECKOUT_ELEMENT_REGISTRY` (entries whose `steps` include `personal-info`); **mouse-move** events with surface-relative `x/y` paths; **scroll** events with `depth` (0–100). Sessions are generated **already finalized** (terminal outcome + `finalized_at` set), so they bypass the lazy/derived sweep entirely. *(Exact anchor sourcing and per-type event counts are confirmed in Part 2.)*
- **Simulation endpoint (`app/api/checkout-heatmap/simulate/route.js`, new).** `POST` (auth-gated) → generate + bulk-write to the resolved sim schema, returns `{ ok, count }`; `DELETE` (auth-gated) → `clearCheckoutHeatmapSessions({ schema: simSchema })`, returns `{ ok }`; `GET` → `{ count }` of sim sessions for the dashboard status line. Auth mirrors the config route (`isAuthorizedToken` + `Authorization: Bearer <token>`).
- **Viewer reads sim (`app/checkout/[sku]/heatmap/page.jsx`).** Read a `source` param alongside `step`/`view`/`type`/`outcome`/`from`/`to`; when `source=sim`, the sessions fetch becomes `GET /api/checkout-heatmap?source=sim`. All downstream filtering/rendering is unchanged.
- **Real-data GET accepts the source enum (`app/api/checkout-heatmap/route.js`).** `GET(request)` reads `?source=` and calls `readCheckoutHeatmapSessions({ schema: resolveHeatmapSchema(source) })`; with no param it behaves exactly as today (base schema). `POST`/`DELETE` are unchanged (real-data only).
- **Dashboard Simulation section (`app/dashboard/DashboardClient.jsx`).** A new `<section data-dashboard-section="simulation">` between the Heatmap and Report sections: a status line driven by `GET /simulate` ("N simulated sessions ready" / "No simulation data"), a **Generate** button (`POST /simulate` with the bearer token; shows progress; refreshes the count), a **Discard** button behind a confirmation overlay (mirrors the Clear-data pattern; `DELETE /simulate`), and a **View Simulation** button that `window.open`s `/checkout/001/heatmap?step=personal-info&source=sim` in a new tab. No step/timeframe/outcome controls (they don't apply); the viewer keeps its own desktop/mobile + clicks/moves/scrolls toggles.
- **Test teardown (`tests/global-teardown.ts`).** Also truncate `heatmap_test_sim` after the suite so simulated rows never persist across runs (guarded, like the existing `heatmap_test` wipe — never touches `public`/`heatmap_sim`).

### Files expected to change / be added

- **New:** `lib/prototype/checkoutHeatmapSimulator.js` (generator); `app/api/checkout-heatmap/simulate/route.js` (GET count / auth-gated POST generate / auth-gated DELETE discard); the dashboard Simulation section (inline in `DashboardClient.jsx` or a small extracted component).
- **Changed:** `scripts/db-setup.mjs` (sim schemas in `SCHEMAS`); `lib/prototype/db.js` (`resolveHeatmapSchema` + allowlist); `lib/prototype/checkoutHeatmapStore.server.js` (optional `schema` arg on reads/clear + a bulk-insert function); `app/api/checkout-heatmap/route.js` (`GET` reads `?source=`); `app/checkout/[sku]/heatmap/page.jsx` (read `source`, thread into the fetch); `app/dashboard/DashboardClient.jsx` (Simulation section); `tests/global-teardown.ts` (wipe `heatmap_test_sim`); `Documentation/DATA.md` (sim-schema note).
- **Unchanged contract:** all real-data paths (`GET/POST/DELETE /api/checkout-heatmap`, `/ingest`, `/query`, `/sweep`, `/cleanup`, `/config`) behave exactly as today when no `source` param is present; capture and the live Shop are untouched (simulation is dashboard-only, server-generated).

### Known test impacts (for `milestone-test-planning`)

- **No existing test should change behavior** — simulation is additive and isolated; real-data tests run against the base schema with no `source` param. Confirm the parameterized store still defaults to `SCHEMA` (a regression risk if a default is missed).
- **New coverage the plan must add:** (1) `POST /simulate` with a valid token generates sim sessions in the sim schema while real-data `GET` (no source) stays empty/unchanged — proves isolation; (2) `POST /simulate` without/with a wrong token is rejected; (3) `DELETE /simulate` wipes the sim schema and leaves real data intact; (4) the viewer with `source=sim` renders sim sessions (and real data is unaffected); (5) the generated mix is approximately right (outcome split within tolerance — a coarse distribution check, not exact counts). Tests run against `heatmap_test` + `heatmap_test_sim`.

### Implementation plan (phased — each part ends with a manual check)

M6.1 is delivered in three sequential parts; the next part does not begin until the current one's manual check is confirmed (same discipline as M2–M6).

**Part 1 — Sim schema + parameterized store.** Add the sim schemas to `db-setup.mjs`; add `resolveHeatmapSchema(source)` + allowlist; give the store reads/clear an optional `schema` arg and add the bulk-insert function. No generator, no UI. *Manual check:* run `db-setup` → the sim schema exists with empty `sessions`/`events`; a one-off store call writes a row to the sim schema and `readCheckoutHeatmapSessions()` (default) still reads only real data.

**Part 2 — Generator + endpoints + viewer reads sim.** Build `checkoutHeatmapSimulator.js`, the `/simulate` route (GET/POST/DELETE, auth-gated mutations), and wire the viewer's `source=sim`. *Manual check:* `POST /simulate` (with token) → ~1500 sim sessions written; open `/checkout/001/heatmap?step=personal-info&source=sim` → clicks/moves/scrolls render across desktop + mobile; real-data heatmap (no source) is unchanged; `DELETE /simulate` → sim view empties, real data intact.

**Part 3 — Dashboard Simulation section + close.** Add the Simulation section (status / Generate / Discard-with-confirm / View Simulation); update `tests/global-teardown.ts`; add the new tests; run close gates. *Manual check:* from the dashboard, Generate → status shows ~1500 → View Simulation opens the populated viewer → Discard (confirm) → status shows none, real data untouched; full suite green; close gates logged (`milestone-doc-review`, tech-debt review, agent review, `DATA.md`/`FUTURE_THIRD_PARTY_INTEGRATION.md` review, committed/clean tree, `milestone-prereqs` → READY).
...
- **Screenshot parallelisation — DONE:** `screenshots/route.js` captures all pages concurrently (`Promise.allSettled`). **Post-M7 hotfix (2026-05-31):** Playwright/`@sparticuz/chromium` replaced with ScreenshotOne external API; route switched to Edge runtime — Vercel Hobby compatible.
...
- **Vercel-compatible screenshots — REPLACED (post-M7 hotfix, 2026-05-31):** The `playwright-core`/`@sparticuz/chromium` Playwright approach was replaced. `screenshots/route.js` now runs on Edge runtime (`export const runtime = "edge"`), calls ScreenshotOne (`https://api.screenshotone.com/take`) via `fetch`, waits on `[data-heatmap-session-count],[data-heatmap-checkout-ready]` selector, and converts the PNG response to base64 via `arrayBufferToBase64`. Auth uses `dashboardAuth.edge.js`. Requires `SCREENSHOTONE_ACCESS_KEY` env var. `playwright-core` and `@sparticuz/chromium` remain in `package.json` and `next.config.mjs` but are now unused by any production route — see tech debt. `full_page=true` is passed to ScreenshotOne so the entire scrollable page is captured, not just the viewport — eliminates the "bottom cut off" problem without shrinking the capture.
- **Screenshot preview UI (post-M7 hotfix, 2026-05-31):** Each screenshot in `ReportClientPage` renders as a scaled preview (`max-h-80 object-contain object-top`) that fits the report width cleanly. Clicking a preview opens a `ScreenshotModal` — a fixed full-screen overlay showing the full-resolution image; click outside or press × to close.
