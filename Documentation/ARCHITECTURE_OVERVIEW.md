...
- **Out of scope (POC deferral):** raw-data export UI (dropped — the backend queries Postgres directly); real account-based auth / audit log / RBAC (M8); durable multi-instance config service + concurrent-admin safety (M8); the real report pipeline (M7); dashboard responsive/mobile layout and UI polish; admin-editable drop-off threshold and event-level throttle (display-only in M6).
...
**Part 4 — Dashboard shell + auth gate + Data section.** Build `/dashboard?token=`, Shop-styled, narrow/centered, desktop-only, with the three section headers. Implement the **Data** section: 8 config items (steps, capture-window timeframe, drop-off threshold display-only, element types, event types, sampling presets 1/10/50/75/100%, desktop/mobile, clear-data) with **staged edits + one Save**; **Clear-data** as an immediate action behind a confirmation pop-up (exempt from Save). *Manual check:* open with the token → dashboard renders (wrong token → blocked); toggle steps/types/sampling + Save → config row updates and capture changes on the next visit; Clear-data pop-up → confirm → all data wiped.
