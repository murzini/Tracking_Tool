...
- **Last closed: M5 — Login Step and Individual Session Attribution (2026-05-26).** Parts 1–3 done. Suite: 58/58 active tests passing. All close gates met. Full detail: the **M5** subsection under Future Milestones.
- **Active: M6 — Admin Dashboard — IN PROGRESS. Parts 1–4 done (2026-05-27); Parts 5–6 to follow.** P1: outcome unification (`advanced`→`completed`). P2: `heatmap_config` Postgres table + config API + `dashboardAuth.js`. P3: capture gated by runtime config. P4: dashboard UI built — single-block layout (Data/Heatmap/Report in one rectangle), MultiSelect dropdowns for Steps/Event Types/Element Types, "Data collecting timeframe" preset dropdown (Today/Last week/Last month/Custom with popover date pickers), **"Drop off timeframe" configurable preset dropdown** (10s/30s/1m/2m/5m — now fully wired end-to-end via `config.inactivityMs`), Sampling Rate select, staged Save, Clear-data confirmation. Test 54 added in `m6-dashboard.spec.ts`. **Suite: 64 tests total; Test 54 step assertions need update to MultiSelect API before running.** ⚠ P4 code + docs not yet committed. **Parts 5–6 remaining:** Heatmap section + viewer outcome filter + timeframe (P5); header removal + Report placeholder + close (P6).
...
- **Where things live in this doc:** most recently closed scope → the **M5** section; settled per-milestone decisions → the **M1–M5** sections (history, marked `STATUS: CLOSED`); cross-cutting debt → the **Tech Debt** register; not-yet-started work → **Future Milestones** M6–M8; speculative ideas → **Potential post-MVP items**.
...
Tech debt M6 is likely to introduce, identified upfront so implementation can plan for it. Outcomes to be marked at M6 close.
- **Dashboard config storage is ephemeral.** The admin dashboard stores element-type, event-type, and sampling-rate toggles in-memory or as a simple JSON file (not a durable config DB). A server restart loses the settings. Acceptable for POC; real deployment needs a persisted config layer (database, feature-flag service, or Vercel KV) so settings survive server restarts and scale to multiple instances. Deferred to M8 integration.
