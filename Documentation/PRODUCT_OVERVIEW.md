...
- ~~**`source`→schema resolution must stay allowlisted (critical).**~~ **Resolved in M6.1.** `resolveHeatmapSchema` in `lib/prototype/db.js` maps the `source` enum (`real`/`sim`) to a concrete schema name and throws on anything else — a raw schema name never reaches SQL. Closed before M6.1 completed.
...
- ~~**Heatmap screenshot capture approach TBD (critical if wrong choice).**~~ **Resolved (post-M7 hotfix, 2026-05-31).** Playwright was chosen in Part 5 but replaced with ScreenshotOne (external API) to fit Vercel Hobby's 10s limit. `screenshots/route.js` runs on Edge runtime; `SCREENSHOTONE_ACCESS_KEY` required in Vercel env vars.
...
- ~~**Screenshot capture is sequential — report takes 45–90s locally.**~~ **RESOLVED 2026-05-29:** route captures all pages concurrently via `Promise.allSettled`. **Post-M7 hotfix (2026-05-31):** Playwright replaced with ScreenshotOne — parallelisation is still in place via concurrent HTTP requests to ScreenshotOne, not browser pages.
...
- **`playwright-core` + `@sparticuz/chromium` deps are now unused in production (post-M7 hotfix, 2026-05-31).** Both remain in `package.json` `dependencies` and in `next.config.mjs` `serverComponentsExternalPackages`, but no production route imports them (screenshot route switched to ScreenshotOne/Edge). `@playwright/test` in devDeps is still needed for the e2e suite. Non-critical — safe to remove in a future cleanup pass.
- **`SCREENSHOTONE_ACCESS_KEY` must be set in Vercel env vars for screenshots to work.** The route returns a 500 if the key is missing; the report renders without images. Operational prerequisite before going live.
- **No automated tests for screenshot route behavior or report screenshot UI (post-M7 hotfix, 2026-05-31).** `screenshots/route.js` calls ScreenshotOne (external API) — unit tests would require mocking `fetch` + `process.env`, and E2E tests require a live `SCREENSHOTONE_ACCESS_KEY` in the test environment (adds cost per run). Untested gaps: `full_page=true` param, auth gate (401), missing key (500), missing base URL (500), partial capture returning `{ ok: true, screenshots }`. The report UI modal (click-to-zoom) and scaled preview are also untested. Accepted for the POC — add coverage at M8 when a real key can be included in CI.
