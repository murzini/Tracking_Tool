...
- **Active milestone: M4 — Extended Interaction Capture.** Parts 1–8 done. Suite: **52/52 active tests passing** (Tests 36 + 44 are `test.fixme`, deferred to M5). Both critical tech-debt items (event-delivery reliability, event-volume vs the free tier) are resolved. Remaining to close M4: the close gates (`milestone-doc-review`, agent review, `milestone-prereqs` → READY). Full detail: the **M4** subsection under Future Milestones.
...
- **Visibility observer thrashing + zero-click sessions never finalize.** *(Surfaced 2026-05-25, session-merge diagnosis; two deferred follow-ups to the activity fix.)* The `element-visible`/`element-hidden` IntersectionObserver fires in large bursts even with no user input (elements oscillating around the 50% threshold), inflating stored event volume. The 2026-05-25 fix stopped those events from counting as *activity* (so sessions idle out correctly), but did **not** quiet the observer itself — worth throttling/hysteresis later. Separately, a **zero-click** session never finalizes (the drop-off finalize requires ≥1 click — an M1 rule), so it stays `in-progress` and `clearResumeRef()` (only called on finalize) never runs, leaving the resume ref to linger. Harmless now that visibility no longer refreshes the resume clock, but allowing no-click bounces to finalize (M4 already records bounces) would be a cleaner second safety net. **Non-critical.**
...
1. **FIRST TASK — Fix the session-merge bug (deferred from M4).** *Do this before the login-step work — it touches the same session lifecycle.*
   ...
   - **Partial change already in M4 (kept, insufficient alone):** visibility events no longer reset activity (`recordSessionEvent(..., { resetActivity:false })` in `checkoutHeatmapClient.js`). The reproducing test (**Test 44**, `m4-session-signals.spec.ts`, currently `test.fixme`) shows the merge **still happens** after this change — at least one more passive source still refreshes the clock. To identify it, add a quick diagnostic to Test 44 (before the reload, log the active session's event types + the resume ref's age via `page.evaluate`); likely culprits are scroll/mouse-move firing without genuine intent, or focus events.
   - **Recommended fix:** make **only deliberate user input** refresh the resume/idle clock; identify and exclude the remaining passive source(s). Decide whether scroll/mouse-move should count, or whether resume should key off a discrete "last meaningful interaction." Then enable Test 44 (remove `fixme`) and re-verify. Also address the two deferred follow-ups: visibility-observer thrashing (throttle/hysteresis) and zero-click sessions never finalizing (so `clearResumeRef` never runs). See Tech Debt → Non-critical.
   - **Also enable Test 36 (remove `fixme`) when the fix lands.** Test 36 (zero-interaction bounce) was failing at M4 close (2026-05-26) because zero-click sessions never finalize — `clearResumeRef` is only called on finalize, which requires ≥1 click (M1 rule). The fix for session-merge should resolve this as a side effect; if not, address it explicitly. See `TEST_CASES.md` → Test 36.

2. **Model-selector helper agent.** A repo/user agent that, given the task at hand, recommends which model to switch to (e.g. Opus for hard reasoning/architecture/tricky debugging, Sonnet for everyday coding, Haiku for simple/fast edits) and how to switch (`/model`). Open decisions to confirm at M5 start: **scope** (this project's `.claude/agents/` vs user-global `~/.claude/agents/`) and the **tier mapping** (3-tier Opus/Sonnet/Haiku vs Opus/Sonnet only). Not heatmap-specific — a workflow helper.
