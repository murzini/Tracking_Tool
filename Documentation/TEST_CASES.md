...
- **Test 36 — Zero-interaction bounce.** ⏸️ **`test.fixme` — DEFERRED TO M5.** *Implemented Part 5 (2026-05-24) — `tests/e2e/m4-session-signals.spec.ts`.* Open a step and leave with no interaction (`pagehide`) → the exit beacon commits the bare session; a forced sweep should finalize it as `outcome:"abandoned"`. **Currently fails:** zero-click sessions never finalize because `clearResumeRef` is only called on finalize, which requires ≥1 click (M1 rule) — so the session stays `in-progress`. Deferred to M5 with the session-merge fix. **Enable (remove `.fixme`) when the M5 fix lands.** Not counted in the green total.
...
- **Test 44 — Visibility events are not activity (session-merge regression guard).** ⏸️ **`test.fixme` — DEFERRED TO M5.** *Added 2026-05-25 — `tests/e2e/m4-session-signals.spec.ts`.* Reproduces the session-merge bug: one real click sets the resume/idle clock, then only visibility events fire (a transform toggle crosses the observer threshold — no scroll/click) for longer than X; on reload the visit must start a **new** session id (not resume). **Currently fails** even with the M4 partial change (visibility ≠ activity) — another passive source still refreshes the clock — so it's marked `test.fixme` to keep the suite green. **Enable (remove `.fixme`) when the M5 session-merge fix lands** (see `PRODUCT_OVERVIEW.md` → M5 scope). Not counted in the green total.
