...
### Current (M1) — local JSON file

Sessions are persisted to a local JSON file at `.m1-data/checkout-heatmap-sessions.json` (gitignored). The file survives server restarts. All reads and writes go through `lib/prototype/checkoutHeatmapStore.server.js`, which handles atomic writes via a `.tmp` rename pattern to prevent corruption.

This is a throwaway store — data is cleared frequently during development via `DELETE /api/checkout-heatmap`. It is not intended for production use.

### M3 — Neon Postgres (current)

**Realized in M3.** The local JSON file store has been replaced with Neon (serverless Postgres) — see the **M3 schema — Postgres (Neon)** section below for the live `sessions` + `events` table definitions. The API contract (`GET`, `POST`, `DELETE /api/checkout-heatmap`) is unchanged — only the store implementation swapped. Dev/test run locally against Neon via the pooled connection string; the Vercel deploy itself is deferred to M8. The M1 JSON-store description above is retained as historical context.
...
Each entry in `session.clicks[]` represents one click or tap.
...
The `.m1-data/checkout-heatmap-sessions.json` file wraps sessions in an envelope:
...
## M3 schema — Postgres (Neon)

M3 replaces the local JSON file store with two Postgres tables. The `GET / POST / DELETE /api/checkout-heatmap` API contract is unchanged.
...
| `exit_reason` | TEXT | how the visitor left a non-completed step: `idle` \| `nav-click` \| `back` \| `left-browser` — new column, M4 Part 5. **`in-progress` + `exit_reason` can co-exist, and this is intentional (decided 2026-05-24):** the exit beacon records *how* the visitor left while the session stays `in-progress` because they may return within X. If X elapses with no return, the sweep finalizes it (`abandoned`) keeping this `exit_reason`. So `in-progress` + `left-browser` reads as "visitor left the page; not yet finalized; may still return within X." |
...
Tests run against the `heatmap_test` Postgres schema (same Neon DB). The playwright runner sets `HEATMAP_DB_SCHEMA=heatmap_test`; `lib/prototype/db.js` reads this env var (default: `public`).
