...
| M7 (demo) | Demo schemas added: `heatmap_demo` (live) and `heatmap_test_demo` (test runner), same `sessions`/`events` tables, no `heatmap_config`. Holds a **frozen copy** of the 22 real sessions (1986 events) for the throwaway demo — copied from `public` by `scripts/freeze-demo-sessions.mjs` (idempotent, non-destructive). Reached via `source=demo` (`resolveHeatmapSchema`). Kept separate from `heatmap_sim` so the demo set never overlaps the 1500-session sim set. No schema change to the tables themselves. |
